...
Creates a draggable, borderless 320x200 live thumbnail using a DWM
thumbnail host window. Press Escape to quit.

Note: many fullscreen games won't produce a DWM thumbnail (thumbnail may be black
or DwmRegisterThumbnail may fail). This script will fall back to window DC
if DWM thumbnailing is unavailable.
"""
...
def capture_scaled_dwm(hwnd, dst_w=320, dst_h=200):
    # Try DWM thumbnail via a small host window
    if t.dwmapi:
        try:
            raw, sw, sh = t.capture_dwm_thumbnail(hwnd, thumb_w=dst_w, thumb_h=dst_h)
            # capture_dwm_thumbnail already returns the requested size
            # raw is BGRA top-down
            mv = memoryview(raw)
            # Convert to RGB
...
def run_live_dwm(title='StarCraft II', thumb_w=320, thumb_h=200, interval_ms=150):
    ...
    def update_frame():
        ...
        try:
            ...
            rgb, w, h = capture_scaled_dwm(hwnd, dst_w=thumb_w, dst_h=thumb_h)
            if have_pil:
...
def main():
    ...
    args = ap.parse_args()
    run_live_dwm(title=args.title, thumb_w=args.width, thumb_h=args.height, interval_ms=args.interval)
